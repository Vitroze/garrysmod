...
function PANEL:DropAction_Normal( Drops, bDoDrop, Command, x, y )
	...
	if ( disty < 0 && self.bDropTop && ( drop == 0 or math.abs( disty ) > h * 0.1 ) ) then drop = 8 end
	if ( disty >= 0 && self.bDropBottom && ( drop == 0 or math.abs( disty ) > h * 0.1 ) ) then drop = 2 end
	if ( distx < 0 && self.bDropLeft && ( drop == 0 or math.abs( distx ) > w * 0.1 ) ) then drop = 4 end
	if ( distx >= 0 && self.bDropRight && ( drop == 0 or math.abs( distx ) > w * 0.1 ) ) then drop = 6 end
	...
	for k, v in pairs( Drops ) do
		...
		if ( ( Command && Command == "copy" or ( IsValid( v:GetParent() ) && v:GetParent().GetReadOnly && v:GetParent():GetReadOnly() && v:GetParent():GetReadOnly() != self:GetReadOnly() ) ) && v.Copy ) then v = v:Copy() end
